...
(define (my/apply procedure arguments env)
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (display "eval compound")
         (eval-sequence
          (procedure-body procedure)
          (extend-environment (procedure-parameters procedure)
                              (list-of-args arguments env
                                            (procedure-parameters-mode procedure))
...
(define (list-of-args args env modes)
  (map (lambda (arg mode)
         (cond ((eq? mode 'strict) (eval arg env))
               ((eq? mode 'lazy) (delay-it arg env unmemoized/force-it))
               ((eq? mode 'lazy-memo) (delay-it arg env memoized/force-it))
               (else
                (error "LIST-OF-ARGS: Unknown parameter mode" mode))))
       args modes))

(define (actual-value exp env)
  (force-it (eval exp env)))

(define (delay-it exp env force-it)
  (list 'thunk exp env force-it))

(define (force-it obj)
  (if (thunk? obj)
      ((thunk-force-procedure obj) obj)
      obj))
...
(define (thunk-force-procedure thunk) (cadddr thunk))
...
(define (make-procedure parameters body env)
  (let ((parameter-symbols (map (lambda (parameter)
                                  (if (symbol? parameter)
                                      parameter
                                      (car parameter)))
                               parameters))
        (parameter-modes (map (lambda (parameter)
                                (if (symbol? parameter)
                                    'strict
                                    (cadr parameter)))
                              parameters)))
...
(define (procedure-parameters p)
  (list-ref p 1))
(define (procedure-parameters-mode p)
  (list-ref p 2))
(define (procedure-body p) (list-ref p 3))
(define (procedure-environment p) (list-ref p 4))
